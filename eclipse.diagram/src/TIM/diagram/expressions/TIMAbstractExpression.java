 ...
 * @generated
 */
public abstract class TIMAbstractExpression {
	...
	*/
	protected void setStatus(int severity, String message, Throwable throwable) {
		String pluginID = TIMDiagramEditorPlugin.ID;
		this.status = new Status(severity, pluginID, -1, (message != null) ? message : "", throwable); //$NON-NLS-1$
		if (!this.status.isOK()) {
			TIMDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
					throwable);
		}
	}
	...
	* @generated
	*/
	protected TIMAbstractExpression(String body, EClassifier context) {
		myBody = body;
		myContext = context;
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				TIMDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
